    ...
    <div style={{ background:"#0D1119", border:"1px solid #1A2235", borderRadius:14, overflow:"hidden" }}>
      <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", padding:"10px 14px", borderBottom:"1px solid #1A2235" }}>
        <div style={{ display:"flex", alignItems:"center", gap:8 }}>
          <BtcLogo size={20} />
          <span style={{ fontSize:"0.78rem", fontWeight:600, color:"#DDE4F0" }}>BTC / USD</span>
        </div>
...
function ShareModal({ position, onClose }) {
  ...
  const shareToX = () => {
    ...
    const text = `I predicted BTC ${dir} on ArcGM 🎯\n${position.winning ? "currently winning" : "open position"} · ${position.streak}d GM streak · ${getMultText(position.mult)} multiplier\narcgm.vercel.app\n#ArcGM #ArcNetwork #Predict`;
...
export default function ArcGM() {
  ...
  // ── PREDICTION HISTORY FROM EVENTS ──
  ...
  const fetchHistory = useCallback(async () => {
    ...
    try {
      ...
      const logsData = await rpcCall("eth_getLogs", [{
        ...
        fromBlock: "0x0",
        toBlock: "latest",
        topics: [PREDICTED_SIG, null, paddedAddr]
      }]);
      ...
      const results = await Promise.all(roundIds.map(async (roundId) => {
        try {
          ...
          // getUserEntry selector: 0x37eaea2c
          // getRoundInfo selector: 0xc57f2dfd
          ...
          // getRoundInfo returns: assetId, roundType, startTime, endTime, snapshotPrice, currentPrice, priceIsUp, finalized, resultUp, noContest
          const roundType = Number(BigInt("0x" + iHex.slice(64,128)));
          ...
          return { roundId, amount, isUp, claimed, winning, estPayout, endTime, snapPrice, finalized, resultUp, noContest, roundType, canFinalize, canClaim, won, lost };
        } catch { return null; }
      }));

      const valid = results.filter(Boolean).sort((a,b) => Number(b.roundId - a.roundId));
      setHistory(valid);
  ...
  const handlePredict  = (isUp) => {
    ...
    writePredict({ address:CONTRACT_ADDRESS, abi:CONTRACT_ABI, functionName:"predict", args:[0, roundType, isUp, parseUnits(entryAmount,6)], chainId:arcTestnet.id });
  };
  const handleClaim    = (roundId) => writeClaim({ address:CONTRACT_ADDRESS, abi:CONTRACT_ABI, functionName:"claim", args:[roundId], chainId:arcTestnet.id });
  const handleFinalize = (roundId) => writeFinalize({ address:CONTRACT_ADDRESS, abi:CONTRACT_ABI, functionName:"finalizeRound", args:[roundId], chainId:arcTestnet.id });

  const handleSwitchNetwork = async () => {
    try { await window.ethereum.request({ method:"wallet_switchEthereumChain", params:[{ chainId:"0x4cef52" }] }); }
    catch { await window.ethereum.request({ method:"wallet_addEthereumChain", params:[{ chainId:"0x4cef52", chainName:"Arc Testnet", nativeCurrency:{ name:"USDC", symbol:"USDC", decimals:18 }, rpcUrls:["https://rpc.testnet.arc.network"], blockExplorerUrls:["https://testnet.arcscan.app"] }] }); }
  };
    ...
    <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", padding:"12px 14px", background:"#111722", borderRadius:10, border:`1px solid ${h.won ? "rgba(0,232,122,0.2)" : h.lost ? "rgba(255,69,97,0.15)" : "#1A2235"}` }}>
      <div style={{ display:"flex", alignItems:"center", gap:10 }}>
        ...
        <div>
          ...
          <div style={{ fontSize:"0.6rem", color:"#3D4F68", marginTop:2 }}>
            Snapshot: {fmtPrice(h.snapPrice)} · Staked: ${fmtUsdc(h.amount)} USDC
          </div>
        </div>
      </div>
      <div style={{ display:"flex", alignItems:"center", gap:8 }}>
        ...
        {h.canFinalize && (
          <button onClick={() => handleFinalize(h.roundId)}
            style={{ background:"rgba(255,184,0,0.1)", border:"1px solid rgba(255,184,0,0.2)", color:"#FFB800", padding:"5px 10px", borderRadius:7, fontSize:"0.65rem", fontWeight:600, cursor:"pointer", fontFamily:"Outfit,sans-serif" }}>
            Finalize
          </button>
        )}
        {h.canClaim && (
          <button onClick={() => handleClaim(h.roundId)}
            style={{ background:"rgba(0,232,122,0.1)", border:"1px solid rgba(0,232,122,0.2)", color:"#00E87A", padding:"5px 10px", borderRadius:7, fontSize:"0.65rem", fontWeight:600, cursor:"pointer", fontFamily:"Outfit,sans-serif" }}>
            Claim
          </button>
        )}
        {(h.won || h.lost) && (
          <button onClick={() => setSharePos({ isUp:h.isUp, snapshot:h.snapPrice, current:null, winning:h.won, streak:myStreak, mult:multVal, wallet:fmt(address) })}
            style={{ background:"rgba(0,212,255,0.08)", border:"1px solid rgba(0,212,255,0.15)", color:"#00D4FF", padding:"5px 10px", borderRadius:7, fontSize:"0.65rem", fontWeight:600, cursor:"pointer", fontFamily:"Outfit,sans-serif" }}>
            📤
          </button>
        )}
      </div>
      ...
      <div className="app">
        ...
        {tab === "predict" && (
          <div style={{ display:"flex", flexDirection:"column", gap:14 }}>
            ...
            {/* Market header — outcome.xyz style */}
                ...
                <div>
                  ...
                  <div style={{ display:"flex", alignItems:"baseline", gap:12 }}>
                    ...
                    <div style={{ fontFamily:"DM Mono,monospace", fontSize:"2rem", fontWeight:500, color:"var(--gold)" }}>
                      ...
                      {priceDiff && <span style={{ fontSize:"0.85rem", fontWeight:600, color: priceDiff.up ? "var(--green)" : "var(--red)", marginLeft:6 }}>{priceDiff.up ? "▲" : "▼"} ${Math.abs(priceDiff.diff/1e18).toFixed(0)}</span>}
                    </div>
                  </div>
                  {priceDiff && (
                    <div style={{ marginTop:4, fontSize:"0.78rem", fontWeight:700, color: priceDiff.up ? "var(--green)" : "var(--red)" }}>
            ...
            {/* BOTTOM TABS — Positions / History / Community */}
            {isConnected && (
                        ...
                        <div style={{ display:"flex", alignItems:"center", gap:10 }}>
                          ...
                          <div>
                            ...
                            <div style={{ fontSize:"0.62rem", color:"var(--muted)", marginTop:2 }}>
                              Stake: ${entryAmt} USDC · Est. payout: <span style={{ color:"var(--arc)" }}>${estimatedPayout} USDC</span>
                            </div>
                          </div>
                        </div>
                        <div style={{ display:"flex", gap:8 }}>
                          {roundFinalized && !entryClaimed && (
                            <button onClick={() => handleClaim(currentRoundId)}
                              style={{ background:"rgba(0,232,122,0.1)", border:"1px solid rgba(0,232,122,0.2)", color:"var(--green)", padding:"7px 14px", borderRadius:8, fontSize:"0.72rem", fontWeight:600, cursor:"pointer", fontFamily:"Outfit,sans-serif" }}
                          ...
                          {!roundFinalized && roundEndTime > 0 && Date.now() > roundEndTime && (
                            <button onClick={() => handleFinalize(currentRoundId)}
                              style={{ background:"rgba(255,184,0,0.1)", border:"1px solid rgba(255,184,0,0.2)", color:"var(--gold)", padding:"7px 14px", borderRadius:8, fontSize:"0.72rem", fontWeight:600, cursor:"pointer", fontFamily:"Outfit,sans-serif" }}>
                              Finalize
                            </button>
                          )}
                          <button onClick={() => setSharePos({ isUp:entryIsUp, snapshot:snapshotPrice, current:currentPrice, winning:entryWinning, streak:myStreak, mult:multVal, wallet:fmt(address) })}
                            style={{ background:"rgba(0,212,255,0.08)", border:"1px solid rgba(0,212,255,0.15)", color:"var(--arc)", padding:"7px 14px", borderRadius:8, fontSize:"0.72rem", fontWeight:600, cursor:"pointer", fontFamily:"Outfit,sans-serif" }}>
                            📤 Share
                          </button>
                        </div>
                      </div>
                  ...
                  {bottomTab === "history" && (
                    <div>
                      <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:12 }}>
                        <div style={{ fontSize:"0.7rem", color:"var(--muted)" }}>All past predictions from chain</div>
                        <button onClick={fetchHistory}
                          style={{ background:"none", border:"none", color:"var(--muted)", fontSize:"0.68rem", cursor:"pointer", fontFamily:"Outfit,sans-serif" }}>
                  ...
                  {bottomTab === "community" && (
                    <div style={{ display:"flex", flexDirection:"column", gap:12 }}>
                      <div style={{ fontSize:"0.75rem", color:"var(--muted)", lineHeight:1.6 }}>
                        Follow ArcGM updates, share your predictions and connect with other players.
                      </div>
                      <a href="https://x.com/mkoneth" target="_blank" rel="noopener noreferrer"
                        style={{ display:"flex", alignItems:"center", gap:12, padding:"14px 16px", background:"#111722", border:"1px solid #1A2235", borderRadius:10, textDecoration:"none", transition:"border-color 0.2s" }}
                      ...
                      <div style={{ fontSize:"0.65rem", color:"var(--muted)", padding:"10px 12px", background:"rgba(0,212,255,0.04)", borderRadius:8, lineHeight:1.6 }}>
                        💡 Share your winning predictions on X to grow the ArcGM community. Tag @mkoneth and use #ArcGM
                      </div>
                    </div>
                  )}
            ...
            <div style={{ display:"flex", flexDirection:"column", gap:8 }}>
              ...
              {leaderboard.length === 0 ? (
                ...
                <div key={e.address} className={`lb-row${e.address?.toLowerCase()===address?.toLowerCase()?" me":""}`} style={{ animationDelay:`${i*0.05}s` }}>
                  ...
                  <div style={{ fontFamily:"DM Mono,monospace", fontSize:"0.68rem", fontWeight:600, textAlign:"right", color: e.mult>=200?"var(--green)":e.mult>=150?"var(--arc)":"var(--muted)" }}>{getMultText(e.mult)}</div>
                </div>
              ))}
            </div>
            <div style={{ display:"flex", flexDirection:"column", gap:10 }}>
              {isConnected && (
                <div style={{ background:"linear-gradient(135deg,rgba(0,212,255,0.08),rgba(0,212,255,0.02))", border:"1px solid rgba(0,212,255,0.18)", borderRadius:12, padding:16, textAlign:"center" }}>
                  ...
                  <div style={{ display:"inline-flex", alignItems:"center", gap:4, background:"rgba(0,232,122,0.1)", border:"1px solid rgba(0,232,122,0.2)", borderRadius:6, padding:"3px 10px", fontSize:"0.68rem", color:"var(--green)", marginTop:6 }}>⚡ {multText} active</div>
                </div>
              )}
              <div classN
